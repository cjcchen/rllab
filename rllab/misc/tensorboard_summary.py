...
class Summary:
    def __init__(self):
        ...
        self._step_key = None
        self._writer = None
        self._histogram_distribute_list = ['normal','gamma','poisson','uniform']
        self._feed={}
    ...
    def record_histogram(self, key, val):
        if str(key) not in self._histogram_ds:
            ...
            self._histogram_summary_op_merge = tf.summary.merge(self._histogram_summary_op)

        self._feed[self._histogram_ds[str(key)]] = val

    def record_histogram_by_type(self,histogram_type, key = None, shape=[1000], **kwargs):
        '''
        ...
        if histogram_type not in self._histogram_distribute_list:
            raise Exception('histogram type error %s' % histogram_type, 'builtin type',self._histogram_distribute_list)

        if str(key) not in self._histogram_ds:
            self._histogram_ds[str(key)] = self.get_histogram_var_by_type(histogram_type, shape, **kwargs)
            self._histogram_summary_op.append(tf.summary.histogram(str(key), self._histogram_ds[str(key)][0]))
            self._histogram_summary_op_merge = tf.summary.merge(self._histogram_summary_op)
    ...
    def get_histogram_var_by_type(self,histogram_type, shape, **kwargs):
        if histogram_type == "normal":
            ...
            return tf.random_normal(shape=shape, mean=mean, stddev=stddev), [mean,stddev]
