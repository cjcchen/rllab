...
class Summary:
    ...
    def record_histogram(self,key, val):
        if str(key) not in self._histogram_ds:
            ...
            self._histogram_summary_op.append(tf.summary.histogram(str(key),self._histogram_ds[str(key)]))
    ...
    def _dump_histogram(self,step):
        if len(self._histogram_summary_op):
            summary_str = self._session.run(tf.summary.merge(self._histogram_summary_op))
